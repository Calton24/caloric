 ...
 * Uses MobileCoreActivityAttributes for the data model.
 *
 * Sections:
...
struct MobileCoreWidgetLiveActivity: Widget {
    var body: some WidgetConfiguration {
        ActivityConfiguration(for: MobileCoreActivityAttributes.self) { context in
    ...
    private func lockScreenView(
        context: ActivityViewContext<MobileCoreActivityAttributes>
    ) -> some View {
        HStack(spacing: 12) {
            Image(systemName: iconName(for: context.attributes.icon))
                .font(.title2)
